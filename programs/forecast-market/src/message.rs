...
pub enum PolyQuestMessage {
    Alive { program_id: Pubkey },
    Message { message: Vec<u8> },
}

impl AnchorSerialize for PolyQuestMessage {
    fn serialize<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
        match self {
            PolyQuestMessage::Alive { program_id } => {
                PAYLOAD_ID_ALIVE.serialize(writer)?;
                program_id.serialize(writer)
            }
            PolyQuestMessage::Message { message } => {
...
impl AnchorDeserialize for PolyQuestMessage {
    fn deserialize_reader<R: io::Read>(reader: &mut R) -> io::Result<Self> {
        match u8::read(reader)? {
            PAYLOAD_ID_ALIVE => Ok(PolyQuestMessage::Alive {
            ...
            PAYLOAD_ID_MESSAGE => {
                ...
                Ok(PolyQuestMessage::Message { message: buf })
            }
            _ => Err(io::Error::new(
...
pub type PolyQuestVaa = wormhole::PostedVaa<PolyQuestMessage>;
...
//         // Verify Payload ID.
//         assert_eq!(encoded[0], PAYLOAD_ID_MESSAGE);
...
//         encoded.push(PAYLOAD_ID_MESSAGE);
